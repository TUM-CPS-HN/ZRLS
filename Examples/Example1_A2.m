...
% Initial center
c = zeros(n, m);
initial_uncertainty_radius_per_element = 0.0;
...
for i=1:totalsteps
    ...
    X_data{i+1,1} = AB * (cartProd(X_data{i},U)) +W+Zeps_LS;

    % 3) ZRLS Data Driven approach
...
for plotRun = 1:length(projectedDims)
    ...
    for iSet = 2:numberofplots
        handleModelArray(iSet-1) = plot(X_model{iSet}, projectedDims{plotRun}, 'b', ...
            'Filled', true, 'FaceColor', [.8 .8 .8], 'EdgeColor', 'b', 'LineWidth', 1.2);
    end

    % Plot reachable sets from data
    for iSet = 2:numberofplots
        handleDataArray(iSet-1) = plot(X_data{iSet}, projectedDims{plotRun}, 'r', 'LineWidth', 1.2);
    end

    % Plot reachable sets from ZRLS data
    for iSet = 2:numberofplots
        handleZRLSArray(iSet-1) = plot(X_data_zrls{iSet}, projectedDims{plotRun}, 'g', 'LineWidth', 1.2);
    end
    ...
    legend([handleX0, handleModelArray(1), handleDataArray(1), handleZRLSArray(1)], ...
        'Initial set ($\mathcal{X}_0$)','Reachable Sets ($\mathcal{R}_k$) from model','Reachable Sets ($\bar{\mathcal{R}}_k$) from data using LS','Reachable Sets ($\hat{\mathcal{R}}_k$) from data using ZRLS','Location','northwest','Interpreter','latex');
     ylim([0.9 2.55]); % Example: from -2 to 5 in x_2
     xlim([0.9 2.8]); % Example: from -2 to 5 in x_2
